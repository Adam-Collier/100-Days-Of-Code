...
### Day 74: March 18th, Saturday

**Todays progress**: Made a fresh repo for FCC Manc, configured the Webpack to handle HTML and CSS, created build, dev and deploy scripts in package.json, created two seperate Webpack configs for the dev and build scripts and split the postcss-loader plugin in a seperate file.

**Thoughts**: Today I did some pair programming via Screenhero and we started to formalise the project in a fresh repo. By slowly implementing the code we constantly made decisions on the necessity of some of the code and better solutions, all taking into account how easy it is to read and understand. We created the deploy script to easily push the build directory files up to a Github pages branch. We made two Webpack config files because in development we wouldn't want to actually create the css file and read the styles from there, instead used Webpack to inline the styles in the head. This involved removing the extract-text-webpack-plugin from the config and using the style-loader instead. For the production config we wanted to create the CSS file in the build folder therefore the extract-text-webpack-plugin would take the css from the loader and create a CSS file in the build directory. We had to do this really because if the path for the CSS was the same in dev and prod it would read it in one and not in the other. It was great to pair programme and using Screenhero made it better to work as a team but also see how other people approach problems and solutions.

### Day 75: March 19th, Sunday
